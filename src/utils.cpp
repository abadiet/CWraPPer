...
void setupHeaderFile(const std::string& filePath, const std::string& origin) {
    ...
    /* Create its source file if it does not exist */
    {
        ...
        const auto hppFilePath = std::filesystem::relative(
            std::filesystem::path(origin), from.parent_path()).string();
        const auto hFilePath = std::filesystem::relative(
            std::filesystem::path(filePath), to.parent_path()).string();
